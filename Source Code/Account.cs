...
namespace Pharmacy_Management_System
{
    public partial class Account : Form
    {
        ...
        Double totalPur, totalRegulerSale, totalWholeSale, totalInc;
        private void btnAccounts_Click(object sender, EventArgs e)
        {
            try
            {
                obj.con.Open();
                obj.cmd = new SqlCommand("select SUM(TotalPurchaseExpenses) from tblStocks", obj.con);
                ...
                if (ob.ToString() == "")
                {
                    ...
                    totalPur = Convert.ToDouble(ob.ToString());

                obj.cmd = new SqlCommand("select SUM(TotalCost) from tblCustomers where WholeSale = 'YES'", obj.con);
                ob = obj.cmd.ExecuteScalar();
                if (ob.ToString() != "")
                    totalWholeSale = Convert.ToDouble(ob.ToString());
                else
                    totalWholeSale = 0;

                obj.cmd = new SqlCommand("select SUM(TotalCost) from tblCustomers where WholeSale = 'NO'", obj.con);
                ob = obj.cmd.ExecuteScalar();
                if (ob.ToString() != "")
                    totalRegulerSale = Convert.ToDouble(ob.ToString());
                else
                    totalRegulerSale = 0;

                totalInc = (totalWholeSale + totalRegulerSale) - totalPur;

                txtNetInc.Text = totalInc.ToString();
                txtTotalRegulerSale.Text = totalRegulerSale.ToString();
                txtTotalPur.Text = totalPur.ToString();
                txtTotalWholeSale.Text = totalWholeSale.ToString();
                obj.con.Close();
            }
            catch (SqlException ex)
            {
