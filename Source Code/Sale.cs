...
namespace Pharmacy_Management_System
{
    public partial class Sale : Form
    {
        ...
        private void cmbProductType_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            txtProductSize.Text = "Medicine Size";
        }
        ...
        void resetSale()
        {
            ...
            cmbProductType.Text = "";
            txtProductName.Text = "Search Medicine";
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    MessageBox.Show("Medicine Name and Type is not Valid", "Error");
                    return;
                }
                ...
                if (stockLimit >= (stock - quantity))
                    MessageBox.Show("This Medicine is Cross the Low Stock Limit","Low Stock");
                try
                {
                    ...
                    if (dtbl.Rows.Count >= 1)
                    {
                        ...
                        if (quantity > stock)
                        {
                            MessageBox.Show("Medicine Name: " + productName + " Stock is not enought??", "Low Stock");
                        }
                        else
                        {
                            obj.con.Open();
                    ...
                    else
                    {
                        ...
                        {
                            obj.con.Open();
                            obj.cmd = new SqlCommand("INSERT INTO tblTemp (ProductName, ProductType, ProductSize, Quantity, UnitPrice, Discount, Price)  VALUES('" + productName + "', '" + productType + "', '" + productSize + "', '" + quantity + "', '" + unitPrice + "', '" + indivisualDiscount + "', '" + price + "')", obj.con);
                            obj.cmd.ExecuteNonQuery();
                            obj.con.Close();
                        }
                    }
        ...
        private void btnNext_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ob.ToString() == "")
                {
                    totalCost = 0;
                    obj.con.Close();
                    MessageBox.Show("Select Medicines First!!","Warning");
                    return;
                }
                else
                {
        ...
        void confirmSale()
        {
            String customerName, customerPhone, customerAddress, payMethod, productName, productType, productSize, time;
            DateTime date;
            int customerId;
            ...
            if (customerName != "" && customerPhone != "")
            {
                try
                {
                    ...
                    for(int i = 0; i < n; i++)
                    {
                        ...
                        obj.cmd = new SqlCommand("INSERT INTO tblSaleProducts (CustomerId, ProductName, ProductType, ProductSize, Quantity, UnitPrice, Discount, Price)  VALUES('" + customerId + "', '" + productName + "', '" + productType + "', '" + productSize + "', '" + quantity + "', '" + unitPrice + "', '" + indivisualDiscount + "', '" + price + "')", obj.con);
                        obj.cmd.ExecuteNonQuery();
        ...
        {
            txtProductName.Text = "";
            cmbProductType.Text = "Medicine Type";
        }
